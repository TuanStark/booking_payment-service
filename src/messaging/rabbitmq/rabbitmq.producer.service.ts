...
export class RabbitMQProducerService {
  ...
  async emitPaymentEvent(topic: string, data: PaymentEventData): Promise<void> {
    try {
      this.logger.log(
        `Publishing payment event to ${topic}: ${JSON.stringify(data)}`,
      );
      ...
      this.logger.error(
        `❌ Failed to publish payment event to ${topic}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async emitBookingEvent(topic: string, data: any): Promise<void> {
    try {
      this.logger.log(
        `Publishing booking event to ${topic}: ${JSON.stringify(data)}`,
      );
      ...
      this.logger.error(
        `❌ Failed to publish booking event to ${topic}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
